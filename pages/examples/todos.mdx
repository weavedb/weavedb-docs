...
In this example, we are defining a schema for the `tasks` collection.
...
<Tabs items={['yarn', 'npm']}>
  ...
  <Tab>
  ```bash copy
  npm install ramda localforage weavedb-sdk buffer ethers@5.7.2 @chakra-ui/react @emotion/react@^11 @emotion/styled@^11 framer-motion@^6
  ```
  </Tab>
</Tabs>
