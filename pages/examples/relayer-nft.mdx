...
3. The owner signs query data (`tokenID`, `Message`) with eip712 and sends it to the relayer with `jobID`. The `signer address` can be later obtained by verifying the eip712 signature.
...
yarn deploy
```
Or you could follow [this guide](/web-console/deploy-database) and use the [Web Console](https://console.weavedb.dev).
...
- [WeaveDB Client]() - to connect with the gRPC node from browsers
- [WeaveDB Node Client]() - to connect with the gRPC node from the serverless api
...
The [access control rules](/examples/relayer-nft#set-up-access-control-rules) previously set will make sure the signer is the owner, and only let messages posted if it's true.
