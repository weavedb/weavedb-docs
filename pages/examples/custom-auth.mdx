...
- `0xDb46d1Dc155634FbC732f92E853b10B288AD5a1d` - Lens NFT Contract

Upload the js file above to IPFS and get the `Content Identifier (CID)`.

You could, for example, use [Pinata](https://www.pinata.cloud/) and they will be immediately available through [the Cloudflare gateway](https://developers.cloudflare.com/web3/how-to/use-ipfs-gateway/).

Let's name the custom authentication flow `custom` (it can be anything).

## 3. Mint/Grant/Burn PKP

[Mint/Grant/Burn a PKP](/sdk/relayers#mint-grant-burn-pkp) on Chronicle testnet using the script below. You need to get some testnet LIT token through [the official faucet](https://faucet.litprotocol.com/).
...
You can also run the sample script from the repo. Just replace your own values for `privatekey` and `ipfsCid` in [custom-auth-pkp.js](https://github.com/weavedb/weavedb-docs/blob/main/examples/custom-auth/pkp-script/custom-auth-pkp.js)
...
<Tabs items={['yarn', 'npm']}>
  <Tab>
  ```bash copy
  yarn create next-app custom-auth
  cd custom-auth
  ...
  cd custom-auth
  npm install add @lit-protocol/lit-node-client ethers weavedb-sdk
  npm run dev
  ```
  </Tab>
</Tabs>
...
- `NEXT_PUBLIC_IPFS_ID` - IPFS content identifier (CID) obtained from Pinata.
...
```
You can view the entire code [here](https://github.com/weavedb/weavedb-docs/tree/main/examples/custom-auth).
