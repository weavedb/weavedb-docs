...
In practice, you would use a verifiable relayer with the [Lit Protocol integration](/sdk/relayers#verifiable-oracles-with-lit-protocol) for more security.

You can also encrypt the notes using [Lit Access Conditions](/examples/lit-protocol).
</Callout>
...
Follow [this tutorial](/examples/relayer-nft#set-up-local-grpc-node) to do so.

## Set up Bundlr

The relayer needs to upload notes to Bundlr and add the `tx.id` to the signed query.

To set up Bundlr, you could also follow [this tutorial](https://docs.bundlr.network/hands-on/tutorials/bundlr-sdk#installing-the-sdk) in their official docs.
