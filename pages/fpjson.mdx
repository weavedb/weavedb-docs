...
Using `mod()` effectively reduces the number of fields users need to provide explicitly, as it allows you to dynamically generate or update values based on pre-defined rules or conditions. This simplifies the user's input and streamlines the data manipulation process.
...
await db.query("add:post", {title: "Test", body: "hello", id: "abc"}, "posts") // Rejected for including an unauthorized field
await db.query("add:post", {title: "Test"}, "posts") // Rejected for failing to include the required `body` field
```
