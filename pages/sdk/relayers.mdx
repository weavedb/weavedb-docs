...
- Set an EVM or Arweave addresses allowed to relay the job in the `Relayers` input field, then click `Add Relayer` button. You can add multiple addresses by clicking `Add Relayer` button. If unassigned, anyone can relay the job.
