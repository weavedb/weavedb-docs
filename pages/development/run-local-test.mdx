...
- Prior to deploying your database instance in production, it is recommended to first deploy it locally on your machine for testing. This will help you catch any issues early on, reducing risks in the production environment.

## Install WeaveDB Tools

Run the following command in your terminal to install `weavedb-tools` globally.
...
Run the following command in the terminal to create a new project, which then navigates to the project's root directory.
...
Replace `your_project` with a name you prefer.
</Callout>
...
In this example, let's build a simple app where users can post and delete messages. Below are the components you would typically configure:
...
To run the tests, open a terminal, navigate to your project's root directory, and execute the following command:
