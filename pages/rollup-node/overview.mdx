...
Inside your folder, create a file named `deploy-warp.js` if it does not already exist.

Copy the script below, paste it in `deploy-warp.js` file, and replace the placeholder values with the necessary information.

```js copy filename="/deploy-warp.js"
const WeaveDB = require("weavedb-node-client")
