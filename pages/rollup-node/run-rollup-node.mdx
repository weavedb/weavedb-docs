...
npm run keygen owner
npm run keygen user
npm run keygen bundler -t ar # for Arweave account
  ```
  </Tab>
</Tabs>
...
4. Locate the `bundler.json` file in your [project](/development/run-local-test#create-a-project), found at `/your_project/.weavedb/accounts/ar/bundler.json`. Copy its entire content and paste it as the value for `bundler` in your `weavedb.standalone.config.js` file.
...
yarn deploy db_name --owner owner
  ...
  <Tab>
  ```bash copy
npm run deploy db_name --owner owner
  ```
  </Tab>
</Tabs>
...
yarn setup db_name --owner owner
  ...
  <Tab>
  ```bash copy
npm run setup db_name --owner owner
  ```
  </Tab>
</Tabs>
