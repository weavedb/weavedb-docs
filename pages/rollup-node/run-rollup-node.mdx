...
Please make sure you have followed and thoroughly tested your database settings as per the [guide](/development/run-local-test) before you proceed to deploy the database to a local rollup node.
...
In this example, we will generate new accounts. Open a terminal, navigate to your [project's root directory](/development/run-local-test#create-a-project), and run the following commands:
...
Run the following command in your terminal to show a list of accounts. Make sure the terminal is in your project's root directory.
...
Open a new terminal and run the commands below to clone the WeaveDB repository and install its dependencies. Make sure the terminal is in a new directory outside of your project's root directory.
...
2. Navigate to your project's root directory in the terminal, then run the following commands:
...
module.exports = {
  db: {
    ...
    name: "Demo Dapp",
    rollup: true, // Set this to true to enable rollup
	plugins: {},
  },
  accounts: { evm: {}, ar: {} },
...
```
- Then, follow the deployment steps outlined earlier in this guide [here](/development/run-rollup-node#deploy-the-database).

- After deployment, you can monitor the rollup blocks in the WeaveDB explorer at [scan.weavedb.dev/node/localhost/db/db_name_prod/blocks](https://scan.weavedb.dev/node/localhost/db/db_name_prod/blocks)

<Callout type="info" emoji="💡">
Make sure you replace `db_name_prod` in the URL with your actual database name to view the corresponding blocks.
</Callout>